...
#include "Core/Log.h"
	...
	#define CORE_INTERNAL_ASSERT_IMPL(type, check, msg, ...) { if(!(check)) { type##LOG_ERROR(msg); CORE_DEBUGBREAK(); } }
	...
	#define ASSERT(...) CORE_EXPAND_MACRO(CORE_INTERNAL_ASSERT__CHECK_MACRO(__VA_ARGS__)(, __VA_ARGS__))
	#define CORE_ASSERT(...) CORE_EXPAND_MACRO(CORE_INTERNAL_ASSERT__CHECK_MACRO(__VA_ARGS__)(CORE_, __VA_ARGS__))
#else
	#define ASSERT(...)
	#define CORE_ASSERT(...)
#endif